...
Delta_Freq_Step=1
tic;
[FDR_dB,ED,VD,OTR,DeltaFreq,~,trans_mask_lte]=FDR_ModelII_app_vec(app,fdr_calc_mhz,array_tx_rf,array_rx_if,array_tx_mask,array_rx_loss,tx_extrap_loss,rx_extrap_loss,Delta_Freq_Step);
toc;
...
ylabel('FDR [dB]')
filename1=strcat('FDR1_NewTx_',num2str(rx_freq_mhz),'.png');
...
figure;
hold on;
plot(ED(:,1)+tx_freq_mhz,-1*ED(:,2)+tx_eirpmhz,'-r')
grid on;
xlabel('Frequency [MHz]')
ylabel('Emission Mask [dBm]')
%axis([7100 7750 -100 0])
filename1=strcat('7GHz_Tx_',num2str(rx_freq_mhz),'_',num2str(rev),'.png');
saveas(gcf,char(filename1))
